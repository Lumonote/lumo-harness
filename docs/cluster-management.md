...
本文件记录 2026-09-07 的开发范围。新增 Lumo 管理入口要求 `LUMO_DEPLOYMENT_MODE=cluster` 和 `LUMO_CLUSTER_STATUS=ready`。Local、Standalone 的原有装配、技能安装及工作流入口保持原路径；新增功能的运行测试和集群联调按用户要求暂缓。
...
源码完成相关编译检查，尚未运行新增功能测试。实际多节点会话失效、数据库并发、浏览器交互及生产依赖联调仍待后续验证。OIDC 与连接器 OAuth 的真实供应商、客户端凭据、回调及用户策略仍须由目标环境配置；桌面设备的真实激活仍依赖原设计中列出的环境输入。
