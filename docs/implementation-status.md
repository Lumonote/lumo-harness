...
4. 具体 SaaS/MCP OAuth 仍需由集成方提供供应商、issuer/端点、已注册回调和 Vault 凭据引用。仓库已校验注册契约，但没有凭空实现第三方授权页、回调、token 交换或 refresh-token 生命周期。
5. 企业 IdP 登录需要经批准的身份源（OIDC/SAML issuer、用户/组声明映射、精确回调、证书/客户端凭据、JIT/停用策略）。当前本地凭据、TOTP 与 Passkey 不会伪装成企业 IdP；在这些输入获批前，该入口保持未启用。
...
- 用户中心增加最近 100 条个人安全事件：登录成功/失败与锁定、登出、会话撤销、改密、TOTP
  ...
  不会以明文或临时密钥降级。企业 IdP 仍未实现，页面不会将验证码、TOTP 或 Passkey 误写为
  企业身份源。
- realm 管理员现在可在账户页管理已有治理用户的启停、部门目录与角色分配。该入口复用
  Governance 的现有 realm-admin 校验；它没有创建登录凭证或发送邀请，所以界面明确标为
  “写入治理目录”，而不是不存在的邀请/入职流程。
