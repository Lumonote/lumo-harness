...
| 对象存储 | **MinIO**（`ctx.datastore.object`） | 桶按 realm 隔离、生命周期分级、纠删码多节点部署；**对象存储 seam 首切片已落地（P2b 项 10/11 地基，2026-08-26）**——`@lumo/object-store` 插件注册 `ctx.objectStore`（realm 前缀隔离 / 内容寻址 sha256 / 缺对象 undefined / 不可达 fail-closed）+ `ctx.spillStore` 收敛（溢出内容对象化，跨节点 resume 任意节点取回）；附件后端待单独切片（[`specs/2026-08-26-object-store-design.md`](./superpowers/specs/2026-08-26-object-store-design.md)） |
