...
| 成本归因与预算策略 | **单截面只保留为 token 截面**（B2 明确要求保留）；成本走并行事件流：`cost_type` 闭集 + trace 归因 + 单一写入者。预算树三档行为（软限额/透支/硬停），默认退化与今天一致 | `architecture` §6.4；实现见 `platform/shared/seam-contracts/{cost-events,budget-policy,metering}.ts` + `platform/dsh-plugins/metering`，设计说明 [`specs/2026-08-25-cost-attribution-design.md`](./superpowers/specs/2026-08-25-cost-attribution-design.md)；待补 PG 侧「本期配额」总额模型、RocketMQ 异步削峰、Doris 聚合 |
