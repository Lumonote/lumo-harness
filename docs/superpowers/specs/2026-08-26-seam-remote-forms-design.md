...
| 3 | `ctx.sessionQuery` | unary | **同一复制日志的读面**：查询永远查「已复制到本地」的段——查询面与日志复制同源，不存在查到半截 | 复制日志投影库（§4.2） | 补 `staleness` 语义：落后超阈值返回显式 `stale` 而非半截结果（§20.1 复制日志行）。**已落地（2026-08-27）**：读面信封契约 `shared/seam-contracts/session-query.ts`（fresh/stale 两臂闭集、`stalenessOf` 阈值纯函数、stale 臂锁 lag ≡ live−replica 自洽）+ `@lumo/session-log` 的 `ctx.sessionLogQuery.queryWithStaleness`（replicaHead 恒取 PG read 最大 seq；liveHead 由调用方显式供给，未供恒 fresh——v1 诚实边界，跨节点下界探测归投影库切片） |
| 4 | `ctx.sessionTitle` | unary | **随 ctx.llm 走网关**：Title Provider 本质是一次 LLM 调用，单截面原则要求它也必须过 LLM 网关（含计量） | LLM 网关 | 无独立动作；接入网关时验证 title 生成的 token 计量进同一截面。**已落地（2026-08-27，验证切片）**：dsh 标题三件套注入 `['sessionTitle','llm','sessions']`——辅助标题调用与主循环共用 `ctx.llm` 同一路由面，Config 成对覆盖 provider/model 指向网关入口即入同一计量截面（emitter=llm-gateway，真 broker 收账见网关首切片 e2e）；集成验证 `platform/dsh-plugins/session-title-gw/__tests__/title-route.spec.ts` 四断言全绿 |
| 5 | `ctx.subagents` | handle | **Scheduler 放置 + 跨节点 session fork**：spawn 变为「放置请求」（在哪个节点执行），执行是远端节点的 session；续跑 = 日志重放 + 写者 fencing（跨节点 fork 公开面已核验 2026-08-27：fork 本体进程内，投影与 seed 重放均在公开面，无需另做会话投影） | Scheduler（§6.2）+ 复制日志 | **跨节点 fork 依赖审计（已核验 2026-08-27）**：`fork()` 本体进程内（`_resolveForkSource` 要求源是目标 store 的 live 实例），但投影（`deriveEventMessage`/`foldSurface`）与 `create({ seed })` 均在公开面——跨节点 fork 以 seed 重放实现、投影复用 dsh 公开导出，**无重实现/漂移风险**。**已落地（2026-08-27）**：one-shot spawn 切片——承载节点 `@lumo/subagent-host`（真 cordis 树 HTTP 放置面 POST /subagent/start\|stop，child 以公开件 `ctx.agents.create` 重建、depth+1、单 turn 驱动、回调结集）+ 父节点 `@lumo/subagent-remote`（Scheduler 放置 → 承载 start → 回调结集 → 终态上报），wire 契约 `shared/seam-contracts/subagent-host.ts`；父侧 dispose 已改走行 6 `JobControlSeam`（`JobRef=(parentSession,node,childId)`，承载侧持本地 Agent handle 执行），创建窗口控制意图不会丢失。fork/continuable 的 seed 传输受 dsh continuable manager 本地物化约束，需独立远程 activation 路径；预算联动放置随后续切片 |
| 6 | `ctx.jobs` | handle | **句柄虚拟化**：`(job) → (session, 执行节点)`；kill/timeout/status 走**全局控制信号**（§7.4 控制命令面：下发与执行分离，执行集群以本地行动为准）；job 结果落地 = 事件流 | 控制信号通道（§7.4）+ Scheduler | **已落地（2026-08-27）**：`@lumo/job-control` 以 PG 作为控制命令邮箱与结果事件真相源：节点观察本地 `ctx.jobs`，登记 `JobRef=(sessionRef,node,jobId)` 与 `job/started\|finished`；授权+幂等 `dispatch` 仅投递给目标 node，执行节点轮询后才调用本地 `ctx.jobs.kill`（`timeout` 同为立即停止，后续是否重派仍归 R2），避免远端代理 OS 句柄。`status` 不入执行 inbox，终态和未知句柄都有闭集结果；真 PG 用例覆盖 mailbox→本地行动→终态事件。恢复语义（超时重派/resume 到何节点）仍绑定 R2 turn 级恢复契约 |
...
| 9 | `ctx.skills` | unary | **本地化优先**：技能经 provisioner 安装到本地再读——「模型可见即日志可重放」要求技能目录在本地是既定的（远端目录状态随时间变化会破坏重放）；远程形态 = 无远程，只有「装没装好」 | 制品注册表 + provisioner（§6.1/§6.5） | 装配层方案：技能目录声明为依赖；本设计的实现责任 = provisioner 确保本地现状。**本地快照切片已落地（2026-08-27）**：`@lumo/skill-local` 启动时校验 `<skill>/SKILL.md` 的路径、非 symlink、frontmatter name、完整根目录 allow-list 与 `sha256`，只把校验后的本地正文注册到 `ctx.skills`；`dsh-node` 配置 `LUMO_SKILL_SNAPSHOT_ROOT` + `LUMO_SKILL_SNAPSHOT` 时禁用可变 filesystem provider，避免 project/user root 覆盖 pin。控制面 Provisioner 的下载/验签/reconcile/心跳仍待实现。 |
...
| 13 | `ctx.workflowEngine` | handle | **FlowEngine 控制面 + 扇出走 subagents**：工作流引擎是控制面进程（独立 Go 服务 §9.2）；其 agent() 调用经 §1 第 5 行的 subagents 路径——引擎跨节点 = 控制面扩容，不是代理 | FlowEngine（§9.2）+ Scheduler | 无独立远程契约；实现依赖第 5 行（subagents）先行。**接线切片已落地（2026-08-27）**：`@lumo/dsh-node` 在 agent 角色覆盖既有 `workflow-worker-thread` 的 provider 为 `lumo-remote`（不另挂第二个 unary engine）；真实 worker-thread `agent()` → `ctx.subagents` → Scheduler/承载节点 → 回调结集已测。DAG 编译、断点续跑、TriggerBus 与独立 Go FlowEngine 控制面仍未实现。 |
...
3. **语言非对称（已核验 2026-08-27）**：dsh 的 fork 是进程内 API——评审 A1 挂账的「若 dsh 未公开跨节点 fork 能力，则平台在 dsh 之外重实现会话投影」**已证伪**：`fork()` 本体进程内（不可跨节点调用），但投影与 replay 入口都在公开面——`surface.ts` 的 `deriveEventMessage`/`foldSurface` 从包 root 再导出，`SessionStore.create(id, { seed })` 是公开的「initial replay or fork history」入口。平台**复用 dsh 唯一投影规则**、以 seed 重放实现跨节点 fork，不重实现、无 `deriveMessages()` 漂移面。剩余需明写的风险是**交付面语义保留**（`seedLength` 亲缘边界、`firstLiveSeq`、`session/end-seed`、`SESSION_FORMAT_VERSION=0` 字节兼容），而非投影正确性。
