...
- [x] `schema.ts`：`STORAGE_PG_SCHEMA_VERSION` + `storage_meta`（单行版本戳，PG 无
  `PRAGMA user_version`）+ `units`/`unit_globals` 元数据表 + `recordTableName`（`u_<unit>_<table>`）。
- [x] `unit.ts`：`PgKvUnit implements KvUnit`——每原语单语句（upsert/delete/select 原子）；
  值存 `TEXT` JSON，读回 `JSON.parse`（坏 JSON → `malformed-medium`，与 sqlite 逐语义一致）。
- [x] `index.ts`：`PgStorageBackend implements StorageBackend`（`kv` facet）+ `apply`
  注册 `ctx.storage.backend` 为 `'pg'`（`inject=['storage']`，镜像 storage-sqlite）。
- [x] 判据：共享契约 5 条（空单元/耐久往返/覆盖删除幂等/版本不匹配拒/close 后拒）+ PG 特有
  （非法名/双开/重开/原型污染键/malformed-medium/注册+dispose）。
- [x] 提交：`feat(storage): PG KV 后端——ctx.storage 收敛到 ctx.datastore.sql（P2b 项 12）`
...
- [x] `attachment.ts` 契约纯函数：附件引用 `sha256:<64hex>` 校验 + `attachmentObjectKey`
  （= `joinRealmKey(realm, content/<hex>)`，复用对象存储键规则）——引用一律对象键，禁止代理。
- [x] `minio-attachment-store.ts`：`MinioAttachmentStore extends AttachmentStore`——复用
...
- [x] 判据：save→ref 是对象键/同内容同键幂等；read 取回并 digest 校验；realm 隔离；缺对象
  undefined→NOT_FOUND；不可达 fail closed。
- [x] 提交：`feat(attachments): 附件 seam 契约 + MinIO Provider——ctx.attachments 对象化（P2b 项 10）`

## 收尾

- [x] dsh 合规；TS 全量 + tsc；真依赖测试两处全绿记录
- [x] 文档收账：architecture §5.1 实现注记 + README 已定案表两行 + seam 远形态第 10/12 行状态