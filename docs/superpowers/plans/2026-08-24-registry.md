...
func (s *Store) Publish(ctx context.Context, raw, sig []byte) (*Record, error) {
	...
	pub, _ := s.trust.Lookup(m.Publisher)
	if bad, ok := trust.ScopesWithin(m.Scopes, pub.MaxScopes); !ok {
		return nil, fmt.Errorf("%w: %s", trust.ErrScopeEscalation, bad)
	}

	digest := objstore.Digest(raw)

	// 先判不可变性再写字节，可以省掉大多数孤儿；但判定必须在事务里再做一次，
	// 因为两次检查之间可能有并发发布。
	if existing, err := s.Get(ctx, m.Name, m.Version); err == nil {
