...
// DDL 建两张表，并把老部署里的**异构同名表**收敛过来。
//
// `revision` 从 0 起（不是 1）：0 的含义是「这一行还没有过一次状态变更」，于是
// 「控制器读到的版本」与库里的版本可以在**未登记**与**已登记但未变更**两种情形下
// 用同一个整数比较——不必再引入一个「是否已登记」的布尔去参与 CAS。
//
// 收敛那一段是 2026-09-20 补的：09-17 那次修的是**代码**（插件不再建这两张表），
// 而**已经建过表的库不会因此变回来**。Compose 不跑 migrate.sh，服务端 DDL 就是这类
// 部署唯一的收敛点，同一批语句也落在 `deploy/migrations/006` 里给跑迁移的部署用。
const DDL = `
